...
public class FeedViewActivity extends Activity{
	...
	public void onCreate(Bundle savedInstanceState){
		...
		try{
			feed_list.setAdapter(new FeedViewItemListAdapter(this, (((TexasStudentMediaApp) getApplication()).getFeedManager().getFeeds()).get(getIntent().getExtras().getInt("feed_index", -1)).getItems()));
			feed_list.setOnItemClickListener(new OnItemClickListener(){
				@Override
				public void onItemClick(AdapterView<?> av, View v, int pos, long id) {
					// Get selected feed item's name
					String feed_name = ((Feed) (((TexasStudentMediaApp) getApplication()).getFeedManager().getFeeds()).get(getIntent().getExtras().getInt("feed_index", -1))).getName();
					Log.v(TAG, feed_name);
